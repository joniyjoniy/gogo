...
public class User_s13t264_06 extends GogoCompSub {
  ...
  private static final int FORM_FOUR      =  2000; // 完4連
  private static final int FORM_PRO_FOUR  =   900; // 仮4連
  private static final int FORM_THREE     =   500; // 完3連
...
//----------------------------------------------------------------
  ...
  int proceedMyHand(int[][] cell, int i, int j, int color) {
    ...
    for (int dx = -1; dx <= 1; dx++) {
      for (int dy = -1; dy <= 1; dy++) {
        ...
        // is33 form 2*2
        if (formerLength == 2 && backLength == 0 && isCompleteRun(formerEnemy, backEnemy, formerWall, backWall)) {
        ...
        if (formerLength == 1 && backLength == 1 && isCompleteRun(formerEnemy, backEnemy, formerWall, backWall)) {
  ...
  int proceedMyHandDir(int fL, int bL, boolean fE, boolean bE, boolean fW, boolean bW) {
    ...
    if (fL == 1 && bL == 1) { return FORM_THREE; }
    return 0;
  }
  ...
  void showRecognition(int i, int j, int dx, int dy, int fL, int bL, boolean fE, boolean bE, boolean fW, boolean bW) {
    System.out.printf("[%2d,%2d],%d,%d,%d,%d,%s,%s,%s,%s\n",
